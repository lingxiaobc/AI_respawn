...
let CONFIG = {
    ...
    chromaKeyEnabled: true,      // App uses the upstream keyer over a solid background.
    ...
    chromaKey: {
        ...
        spill: 1.0           // Remove green excess without mixing contaminated color back in.
    }
};
// ==================================================
...
class VideoProcessor {
    ...
    async init(videoUrl, gzipUrl) {
        ...
        // Decode the initial neutral frame before rendering. Keep its matching pose data.
        ...
        // Seeking explicitly makes the decoded frame available to drawImage even when
        // Chromium has never presented this off-DOM video. Avoid its initial black surface.
        await new Promise((resolve, reject) => {
            this.video.addEventListener('seeked', resolve, { once: true });
            this.video.addEventListener('error', reject, { once: true });
            this.video.currentTime = 2 / VIDEO_FPS;
        });
        this.video.pause();
    ...
    async fetchVideoUtilData(gzipUrl) {
        ...
        if (!response.ok) throw new Error("Avatar data HTTP " + response.status);
        const compressedData = new Uint8Array(await response.arrayBuffer());
        // Some static servers advertise Content-Encoding:gzip and the browser already decodes it.
        const decompressedData = compressedData[0] === 0x1f && compressedData[1] === 0x8b
...
function initChromaKeyGL() {
    ...
    const fragmentShaderSource = `#version 300 es
        ...
        void main() {
            ...
            // RGB distance alone misses dark green pixels around hair and jacket edges.
            float greenExcess = max(0.0, fg.g - max(fg.r, fg.b));
            float greenRatio = greenExcess / max(0.02, fg.g + max(fg.r, fg.b));
            mask *= 1.0 - smoothstep(0.10, 0.50, greenRatio);
...
//        Module["asm"]["stringToUTF8"](jsonString, stringPointer, lengthBytes);
        ...
        // App integration: do not log complete avatar data.
        ...
        if (version_valid == 0) {
            throw new Error("DH_live resource version mismatch")
        }
...
// Animation is restricted to the interior of the face. No head/neck compositing seam.
const bodyBase = document.createElement('canvas');
const bodyBaseCtx = bodyBase.getContext('2d');
const faceLayer = document.createElement('canvas');
const faceLayerCtx = faceLayer.getContext('2d');
const mouthMask = document.createElement('canvas');
const blinkLayer = document.createElement('canvas');
let blinkEpoch = 0;
let attributionCaptured = false;
function ellipseMask(context, x, y, rx, ry) {
    context.save(); context.translate(x, y); context.scale(rx, ry);
    const mask = context.createRadialGradient(0, 0, 0.78, 0, 0, 1);
    mask.addColorStop(0, 'rgba(255,255,255,1)'); mask.addColorStop(1, 'rgba(255,255,255,0)');
    context.fillStyle = mask; context.fillRect(-1, -1, 2, 2); context.restore();
}
async function prepareFaceLayers() {
    ...
    const video = videoProcessor.video;
    const seek = time => new Promise((resolve, reject) => {
        video.addEventListener('seeked', resolve, { once: true });
        video.addEventListener('error', reject, { once: true });
        video.currentTime = time;
    });
    const closedFrame = 65; // verified against the bundled portrait, not arbitrary user assets
    await seek(closedFrame / VIDEO_FPS);
    const pose = dataSets[closedFrame], ref = dataSets[2];
    const context = blinkLayer.getContext('2d');
    context.save(); context.translate(350, 386);
    context.rotate(-(Math.atan2(pose.points[1], pose.points[0]) - Math.atan2(ref.points[1], ref.points[0])));
    context.translate(-(pose.rect[0] + pose.rect[2]) / 2, -pose.rect[1]);
    context.drawImage(video, 0, 0); context.restore();
    const mask = document.createElement('canvas'); mask.width = 720; mask.height = 1080;
    ellipseMask(mask.getContext('2d'), 294, 354, 34, 21);
    ellipseMask(mask.getContext('2d'), 398, 354, 34, 21);
    context.globalCompositeOperation = 'destination-in'; context.drawImage(mask, 0, 0);
    context.globalCompositeOperation = 'source-over';
    await seek(2 / VIDEO_FPS); video.pause(); blinkEpoch = performance.now();
}
function compositeFace() {
    ...
    ctx_video.save(); ctx_video.globalAlpha = blink; ctx_video.drawImage(blinkLayer, 0, 0); ctx_video.restore();
}
...
async function processVideoFrames() {
    ...
    if (deltaTime >= FRAME_INTERVAL) {
        ...
        await processDataSet(currentFrameIndex);
        compositeFace();
        // Key after mouth synthesis, so the face patch cannot reintroduce green.
        if (CONFIG.chromaKeyEnabled && chromaKeyGl) {
            const result = processChromaKey(canvas_video);
            ctx_video.clearRect(0, 0, canvas_video.width, canvas_video.height);
            ctx_video.drawImage(result, 0, 0);
        }
