...
2026-09-09: MiniLive2.js additionally accepts the locally generated PHOTO profile (video, data and mouth ellipse). Photo mode retains the validated static portrait/mouth-only composite and disables actor-specific blinking and chroma key. runtime.js now presents a readable generic resource error for non-Error media events. No WASM/model binary was changed. Text integrity checks normalize CRLF to LF; the qt-loader hash was corrected after verifying its complete retained functions against the pinned upstream source.
