...
approved_plan_digest: sha256:4ef2c3c101f03ce4899b9f5ca640e1e504bfe9aa67ec47956f2b320e5789e3aa
routing_mode: auto
...
- 批准依据：用户“全部批准并执行”，PT-role-resource-pipeline v2（用户明确变更为现有Ubuntu验证）。
- 路由快照：full、高风险、混合。
...
| A-06 | 完整服务/测试资源/依赖锁定 | 现有Ubuntu24.04隔离验证、无GPU启动及检查脚本、进程组内存/阶段耗时报告、运行手册 | A-05 | 本地隔离环境及依赖/浏览器下载；不访问生产、不公开端口 | 消除Windows绝对路径/msedge依赖；本地处理+headless浏览器串行，峰值进程组目标<=1.5GiB，超标不得称服务器通过；记录手机视口而非冒充真机；默认loopback、管理接口最小访问保护；运行文档可复现 | 无隔离权限/内存超限明确未验收，优化本地串行释放或请求权限；不动生产服务器 |
## 任务明细
...
- [x] T-06 | A-06 | 完成Ubuntu适配与交付 | output: 现有Ubuntu24.04隔离验证、无GPU启动及检查脚本、进程组内存/阶段耗时报告、运行手册 | acceptance: 消除Windows绝对路径/msedge依赖；本地处理+headless浏览器串行，峰值进程组目标<=1.5GiB，超标不得称服务器通过；记录手机视口而非冒充真机；默认loopback、管理接口最小访问保护；运行文档可复现
## 发现与变更记录
- 2026-09-05 | T-06 | evidence: type=artifact; locator=AI_output/linux-validation/worker-validation.json; result=现有Ubuntu24.04 CPU回放12阶段到awaiting_review，worker及全部后代含QA浏览器峰值1011118080字节、43.331秒；UI功能通过、中文截图人工复查正常，含模拟客户端的UI整树1751535616字节超限单列保留，不冒充后台通过。
- 2026-09-05 | v2批准：用户明确要求先用现有Ubuntu，不考虑版本；保留v1快照，前五步证据沿用，只调整第六步版本要求。
...
- 2026-09-05 | state: IN_PROGRESS -> COMPLETED | reason: 全部验收通过

## 完成标准
- 六项逐一有证据，真实缓存和模拟测试区分；用户审查不自动代办。
- 现有Ubuntu24.04实测及峰值<=1.5GiB；未验证不能标为完成。
- A-01 | evidence: type=test; locator=packages/role-resource/state.test.ts; result=Ubuntu完整74项测试通过，幂等、独立角色和账本保护通过，历史调用仍9条。
- A-02 | evidence: type=artifact; locator=AI_output/linux-validation/worker-validation.json; result=真实CPU预检及定位通过；缺GLES时原角色暂停未调用供应商，补齐私有依赖后通过。
- A-03 | evidence: type=artifact; locator=AI_output/linux-validation/worker-validation.json; result=原图到资源全12阶段完成，缓存回放未新增付费，当前资源待审查。
- A-04 | evidence: type=artifact; locator=AI_output/linux-validation/work/AI_output/role-ui-qa/report.json; result=Ubuntu网页上传、预览、刷新、拒绝及备注持久化通过，390px无横向溢出，中文截图可读。
- A-05 | evidence: type=test; locator=npm test and python -m unittest discover -s scripts/portrait-motion -p test_*.py; result=Ubuntu74项Node和11项Python测试通过；资源63帧遮罩外变化0、嘴部幅度和浏览器音频静音通过，窄范围review无阻断。
- A-06 | evidence: type=artifact; locator=AI_output/linux-validation/linux-worker-memory.json; result=Ubuntu24.04后台及后代峰值约0.942GiB低于1.5GiB，43.331秒，中文渲染复查通过；另行保留含模拟客户端的UI总内存超标，不代表腾讯云生产验证。
- overall | evidence: type=file; locator=docs/ubuntu-validation-2026-09-05.md; result=六项开发及现有Ubuntu验收证据齐全，记录环境错误和修复、两种内存边界、零新增付费及待人工审查；无部署提交，模板通用验证因仓库缺脚本不冒充通过。
