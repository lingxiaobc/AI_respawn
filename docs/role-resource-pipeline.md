...
> 配置规则已更新：所有项目配置只从仓库根 `.env` 读取；下文历史验证用的Shell变量覆盖已失效。文件缺失直接报错，修改后须重启。新的UI测试安全配置及旧Ubuntu测试限制见 [配置修复说明](env-source-fix.md)。

## 本次交付与证据边界

输入一张图、对应一个角色。后台依次执行：原图基础文件检查 → ZenMux规范化 → 标准图严格质量检查及CPU五官定位/小范围遮罩 → 四态素材 → 配准及63帧局部动画 → 像素和口型审计 → HTML/APNG → 浏览器检查 → ZIP → 等待用户审查。
...
原图仅检查文件大小不超过12MiB、单帧PNG/JPEG/WebP、完整解码及边长512至8192像素，不调用人脸定位模型。通过仅表示文件有效，不表示五官、身份或动画已经合格。

严格检查针对生成后的1024×1536不透明标准图：CPU模型必须检出一个人脸；脚本验证478点、必要坐标在画布内、眼线倾斜、嘴在眼睛下方、对齐关系及人脸清晰度。清晰度用统一256px脸区的Laplacian方差，初始阈值15；双眼中心倾斜阈值0.25弧度，同时保留外眼角斜率不超过0.25的门槛。新构建与缓存复用均执行严格检查及遮罩重叠检测。失败以CANONICAL_INVALID标明对象，暂停在locate阶段，不请求四态素材。

因此有效原图可能先消耗一次规范化请求，再因标准图不合格暂停；不会用原图478点失败直接否决规范化。三张已有标准图已复验通过，但这些是可调启发阈值，不代表遮挡/身份变化都能自动识别。90多岁男性的旧拒绝属于原图门槛前置，不是坏标准图的证据，见[门禁修正记录](canonical-gate-fix-2026-09-06.md)。

定位后继续检查五官区域大小、运动遮罩越界/重叠和配准异常；任何失败暂停该角色。最终检查63帧遮罩外像素零变化、闭/半/开嘴幅度、三个参数独立、音量驱动及静音闭嘴；流畅自然和接缝仍由用户确认。

## 测试

完整串行批处理（2026-09-05补充）：

```sh
node --experimental-strip-types scripts/role-resources.ts batch image/a.png image/b.png
node --experimental-strip-types scripts/role-review-gallery.ts
```

网页仍一次上传一张。CLI逐图创建角色、运行完整12阶段，将本次结果写入角色根目录的`batch-report.json`；任一角色暂停时以退出码1停止后续图片。按图片内容生成批次幂等键，重复执行同图不新建角色、不重发失败请求；暂停角色必须明确`resume <id>`，未知付费记录仍会阻止重发。

统一入口可校验复用`NORMALIZATION_OUTPUT_DIR`（默认`AI_output/normalized`）的旧规范图：绑定原上传文件哈希、保存的PNG像素、提示词版本、结果哈希与尺寸。旧`source.png`是转换后的PNG，不能用其文件字节哈希冒充原上传文件哈希。任何损坏直接暂停。已有规范图复用不增加供应商调用。

结果展示脚本只复制校验通过的独立预览到`AI_output/batch-review`，失败角色仅显示错误；该目录可用只绑定127.0.0.1的HTTP服务打开。它不会把失败角色生成成伪动画，也不会替用户审批。
