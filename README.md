...
这是一个本地、管理员分配账户、半双工的实时语音验证项目：支持多账号登录，全系统同时一场通话。在 Windows Chrome 中自动监听用户说话，连续静音约 1.2 秒后由豆包 Realtime 3.0 流式返回语音并立即播放。

管理员可每批上传 1—3 张照片，一图一角色，在本机依次制作 DH_live_mini 人物资源，再由豆包实时回复音频驱动嘴型。历史角色总数不设上限，SQLite 保存人物、账户、授权、任务与通话元数据。一个用户可选择多个已分配人物；共享人物的用户专属设定互不影响。浏览器本机运行 WASM，无需 GPU 服务器。当前不含声音复刻、RAG、长期记忆、全双工或公网部署。账户说明见 [账户与授权](docs/ACCOUNTS.md)，通话规则见 [批量人物与通话阶段](docs/BATCH_CALL_PHASE.md)，环境恢复见 [照片实时通话说明](docs/PHOTO_REALTIME_LOCAL.md)。
...
新安装首次启动前，需要通过受控进程环境 `ADMIN_INITIAL_PASSWORD` 或不回显的标准输入执行 `npm run account:init`，初始化唯一管理员 `lenox`。已初始化的管理员不会被启动配置覆盖。本次本机工作区已初始化；密码不存于源码或本文。详情与恢复步骤见 [账户与授权](docs/ACCOUNTS.md)。

现在打开页面会先要求登录。管理员进入“用户管理”创建账号并分配人物；照片上传、重试、重命名和默认设定编辑位于“人物管理”。普通用户只可选择已分配的人物通话。以下照片操作说明适用于管理员。
