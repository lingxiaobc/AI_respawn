...
const host=spawn(process.execPath,["--experimental-strip-types",resolve("apps/server/src/server.ts")],{
  windowsHide:true,stdio:["ignore","pipe","pipe","ipc"],env:{...process.env,PORT:String(port),AVATAR_STORAGE_DIR:root,DIAGNOSTICS_DIR:resolve(root,"logs")}});
host.stderr?.resume();
...
async function call(avatar:string){
  ...
  const ws=new WebSocket(base.replace("http","ws")+"/ws?avatar="+avatar,{origin:"http://127.0.0.1:5173"});
